...
int main (int argc, char ** argv) {
	...
	while(1){
		...
		if (PQresultStatus(result)!=PGRES_TUPLES_OK) { SQLERR; AT; goto l2; }
		...
		if (bsearch(hmac_text,hmacs,n_data,2*SHA256_DIGEST_LENGTH,compar)) puts(PQgetvalue(result,0,0));
	PQclear(result); }
	PQclear(result);
	PQexec(db,"close hashes");
	if (PQresultStatus(result)!=PGRES_COMMAND_OK){ SQLERR; AT; goto l2; }
	PQclear(result);
	PQfinish(db);
	if (hmacs) free(hmacs);
	free (key);
